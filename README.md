...
7. 若 GitHub 未能为主任务分配 Ubuntu 托管运行器，失败事件会自动触发
   `daily-update-runner-fallback`，改用独立的 macOS 运行器池完成抓取、发布和国内镜像部署。
...
- 上交所直接读取 ETF 规模报表的 `TOT_VOL`（万份），来源标记为
  `sse_official_shares`；旧的 SCALE/净值反推路径只在直接份额接口
  临时不可用时作为官方兜底。
...
scripts/backfill_etf_shares.py 回补2023-10-23至今的ETF份额历史
